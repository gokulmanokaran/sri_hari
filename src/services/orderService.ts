...
/** Pre-Payment Persistence: Saves order into Supabase BEFORE Razorpay modal opens */
export async function createPendingOrderInSupabase(
  payload: OrderNotificationPayload
): Promise<{ success: boolean; error?: string }> {
  try {
    const supabase = getSupabaseClient();
    if (!supabase) {
      console.warn("[OrderService] Supabase client not initialized for pending order save.");
    ...
    // Check if order already exists to avoid downgrading a 'Paid' order back to 'Pending Payment'
    ...
    if (existing && existing.payment_status && existing.payment_status.startsWith("Paid")) {
      console.info(`[OrderService] Order ${payload.orderId} is already marked Paid in Supabase.`);
      return { success: true };
    }

    const row = {
      id: payload.orderId,
      razorpay_payment_id: payload.paymentId || payload.razorpayPaymentId || null,
      razorpay_order_id: payload.razorpayOrderId || existing?.razorpay_order_id || null,
    ...
    if (error) {
      console.warn("[OrderService] Failed to persist pending order to Supabase:", error.message);
      return { success: false, error: error.message };
    }

    console.info(`[OrderService] 🛡️ Pending order #${payload.orderId} successfully persisted in Supabase before payment.`);
    return { success: true };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.warn("[OrderService] createPendingOrderInSupabase exception:", msg);
    return { success: false, error: msg };
  }
}

/** Update pending order with Razorpay Order ID as soon as generated by backend */
export async function updatePendingOrderRazorpayId(
  orderId: string,
  razorpayOrderId: string
): Promise<void> {
  try {
    ...
    console.info(`[OrderService] Linked Razorpay Order ID ${razorpayOrderId} to storefront order #${orderId}`);
  } catch (err) {
    console.warn("[OrderService] Error linking razorpay_order_id:", err);
  }
}
